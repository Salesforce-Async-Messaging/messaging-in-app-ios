...
1. **Fetch examples before writing code.** Before generating any code that uses SDK
   ...
   If the fetch fails, you may proceed using `reference/api-surface.md` as your
   source of truth for type names and method signatures. Do NOT generate SDK code
   purely from memory -- always verify against either the fetched example or the
...
2. **Minimal changes only.** Make the smallest change to achieve the goal. Do not
   ...
   boilerplate. Exception: delegate implementations, builder subclasses
   (e.g. `NavigationBarBuilder`, `ChatFeedViewBuilder`), and other SDK extension
   classes should always go in their own dedicated file. Other than these, prefer
...
5. **Do NOT generate unnecessary code.** If the example omits a protocol property or
   ...
   Exception: `ChatFeedViewBuilder.accessibilityLabel` is a required property --
   always implement it, returning `nil` if you have no custom label.
...
If you cannot fetch it, use `reference/api-surface.md` as your source of truth.
...
8. If the project uses `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`:
   ...
   - If the deployment target is iOS 17+: any coordinator/view model holding SDK state
     uses `@Observable` + `@State`, not `ObservableObject` + `@StateObject`?
