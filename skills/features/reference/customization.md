...
### Common mistakes
- `accessibilityLabel` is a required protocol property. Always implement it, but return
  `nil` if you have no custom accessibility label: `var accessibilityLabel: AccessibilityLabelClosure? { return nil }`
...
If you cannot fetch the examples, use `reference/api-surface.md` as your source of truth.
...
- If the project uses `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`:
  Mark `init()` as `nonisolated` (required because the SDK's base `init()` is
  `nonisolated`). Inside the `handleNavigation` closure, wrap UIKit access in
  `MainActor.assumeIsolated { }` since `UINavigationItem` is `@MainActor`:
  ```swift
  nonisolated override init() {
      super.init()
      self.handleNavigation = { screenType, navigationItem in
          MainActor.assumeIsolated {
              // UIKit access goes here
          }
      }
  }
  ```
