...
init(configuration: Configuration,
     ...
     createConversationOnSubmit: Bool = true,
...
func client(_ client: ConversationClient, didUpdateConversation conversation: Conversation)
func client(_ client: ConversationClient, didUpdateUnreadMessageCount count: UInt)
```

---

## UserVerification (SMIClientCore)

Returned from `UserVerificationDelegate` to provide authentication credentials.

### Initializer
```swift
UserVerification(customerIdentityToken: String, type: AuthorizationTypes)
```

### Properties
| Property | Type | Description |
|----------|------|-------------|
| `customerIdentityToken` | `String` | The JWT token used to verify the user |
| `type` | `AuthorizationTypes` | The authorization type for the token |

### ChallengeReason

Passed to `UserVerificationDelegate` to indicate why verification is needed.

```swift
ChallengeReason.expired     // Token has expired
ChallengeReason.malformed   // Token is invalid
ChallengeReason.refresh     // Token refresh requested
```

Handle `.expired` and `.malformed` by returning `nil` to stop the retry loop.
...
Most integrations use `.JWT`. Use `.passthrough` only when minting MIAW JWTs
externally (e.g. via Salesforce Mobile SDK).

---

## ChatFeedViewBuilder (SMIClientUI)

Protocol for replacing chat feed views. All three properties are required.
If you have no custom accessibility label, return `nil` from the closure.
...
protocol ChatFeedViewBuilder {
    ...
    var renderMode: RenderModeClosure? { get }          // required
    var accessibilityLabel: AccessibilityLabelClosure? { get }  // required
    var completeView: CompleteViewClosure? { get }       // required
}
```
...
### Key Payload Type Properties

**TextMessage:**
| Property | Type | Description |
|----------|------|-------------|
| `text` | `String` | The message content |
| `citationContent` | `CitationContent?` | Optional citation data |
...
| `asset` | `ImageAsset?` | Optional preview image |
...
| Property | Type | Description |
|----------|------|-------------|
| `text` | `String` | The picker prompt text |
| `choices` | `[Choice]` | The list of selectable items |
