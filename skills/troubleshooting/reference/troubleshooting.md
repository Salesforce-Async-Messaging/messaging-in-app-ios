...
**Fix (iOS 17+):** Replace `ObservableObject` with the `@Observable` macro:
```swift
...
**Fix (iOS 16):** `@Observable` is not available. Keep `ObservableObject` + `@StateObject`
and mark the class and its properties as `@MainActor` explicitly to resolve the isolation error:
```swift
@MainActor
class ChatCoordinator: ObservableObject {
    @Published var transcriptURL: URL?
...
**Cause:** The project uses `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor` (Xcode 26 default)
and a subclass of an SDK type overrides a `nonisolated` method.

**Fix:** Mark the overridden initializer as `nonisolated`. For `NavigationBarBuilder`
subclasses, the `handleNavigation` closure accesses `@MainActor` UIKit properties
(`UINavigationItem`), so wrap UIKit access in `MainActor.assumeIsolated`:
```swift
...
```swift
let key = "salesforce_conversation_id"
