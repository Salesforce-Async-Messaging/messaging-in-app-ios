...
2. **Fetch examples before writing code.** Before generating any code that uses SDK
   ...
   If the fetch fails, you may proceed using the api-surface reference as your
   source of truth for type names and method signatures. Do NOT generate SDK
   code purely from memory -- always verify against either the fetched example
   or the api-surface reference.
...
6. **Line length limit.** Keep all lines under 160 characters. Break long lines.

7. **Handle actor isolation.** If the project uses `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor`
   (Xcode 26 default):
   - Mark overridden initializers and methods as `nonisolated` when subclassing SDK types
     (e.g. `NavigationBarBuilder`).
   - If the deployment target is iOS 17+, use `@Observable` instead of `ObservableObject`
     for any coordinator or view model class that holds SDK state. Use `@State` instead
     of `@StateObject`, and `State(initialValue:)` in `init()` when a non-literal initial
     value is needed. If the deployment target is iOS 16, keep `ObservableObject` +
     `@StateObject` — `@Observable` is not available below iOS 17.
...
Search the project to determine (skip any that the user has already answered):
...
Use that version as the `minimumVersion`. If the fetch fails or times out, check the
releases page instead:
`https://github.com/Salesforce-Async-Messaging/Swift-Package-InAppMessaging/releases`

If neither is reachable, tell the user you need network access to check the version.
Never hardcode an older version or guess.
...
| Reference exists but `productName` is `SMIClientUI` instead of `Swift-InAppMessaging` | Fix the product name in all entries |
...
**Edit 1 — Add `PBXBuildFile` entry.** If the `/* Begin PBXBuildFile section */` markers
already exist, insert the line inside the existing section. If no `PBXBuildFile` section
exists, create it (insert before `/* Begin PBXFileSystemSynchronizedRootGroup section */`
...
If the fetch fails, use the api-surface reference as your source of truth.
...
4. Configuration values are the user's real values, not example placeholders like
   `"00Dxx0000000000"` or `"Your_Deployment_Name"`?
5. The `conversationId` is properly managed -- NOT `UUID()` inline in the
...
6. Only the necessary files were created or edited -- nothing extra was added?
7. The generated code matches the pattern from the fetched ContentView.swift or
   UIKitMIAW.swift example -- no improvisation or invented patterns?
8. The user's existing view hierarchy was not replaced -- the SDK was integrated
   into it?
9. The project's minimum deployment target is iOS 16.0 or later?
